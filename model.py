...
def generate_big(conn, cur, count,mod):
    ...
    if mod == '1':
        ...
        sql = "INSERT INTO readers(name,surname,phone_number) select chr(trunc(65 + random()*25)::int)||" \
              "chr(trunc(65 + random()*25)::int)||" \
              " chr(trunc(65 + random()*25)::int)," \
              "chr(trunc(65 + random()*25)::int)|| chr(trunc(65 + random()*25)::int)|| chr(trunc(65 + random()*15)::int) || chr(trunc(65 + random()*30)::int)," \
